...
def test_plan_command_writes_outputs(tmp_path: Path, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    assert graph["nodes"]
    assert graph["edges"]
    stage_nodes = [node for node in graph["nodes"] if node["node_kind"] == "phase_equilibrator"]
    ...
    assert any(node["node_kind"] in {"gas_buffer", "liquid_buffer"} for node in graph["nodes"])
    assert not any(node["node_kind"] == "conditioning_valve" for node in graph["nodes"])
    assert not any(node["node_kind"] == "recycle" for node in graph["nodes"])
    assert any(edge["parameters"].get("pipe_network") in {"gas", "liquid"} for edge in graph["edges"])
    assert any(
